...
def concatenacao(automato1, automato2): 
    if(type(automato1) is Automato) and (type(automato2) is Automato):
        ...
        automato.add_transicao(automato1.fim, automato.epsilon(), automato2.inicio)
        for i in automato2.lista_transicao:
            automato.add_transicao(i.origem, i.nome, i.destino)
        return automato
    print("\n\n erro, não é autômato\n\n")
...
def uniao(automato1, automato2): 
    if(type(automato1) is Automato) and (type(automato2) is Automato):
        ...
        automato.add_transicao(automato1.fim, automato.epsilon(), automato.fim)
        automato.add_transicao(automato2.fim, automato.epsilon(), automato.fim)            
        return automato
    print("\n\n erro, não é autômato\n\n")
    return False
...
def fecho_kleene(automato1):
    if(type(automato1) is Automato):
        ...
        for i in automato1.lista_transicao:
            automato.add_transicao(i.origem, i.nome, i.destino)
        automato.add_transicao(automato1.fim, automato.epsilon(), automato1.inicio)
        automato.add_estado(automato.letra_estado + str(1+(int(automato1.fim[1:]))))
        automato.add_transicao(automato1.fim, automato.epsilon(), automato.fim)
        automato.add_transicao(automato.inicio, automato.epsilon(), automato.fim)
        return automato
    print("\n\n erro, não é autômato\n\n")
    return False